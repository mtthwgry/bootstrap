...
- Steps assume ordering: `preflight` (platform + Xcode CLT/license) → `homebrew` (installs brew + everything in Brewfile) → `mise` → `dotfiles` → `shell` (login shell → zsh) → `ghostty` → `vscode` → `github` → `claude`. Anything depending on a brew-installed tool must run after `homebrew` (which is also why `lib/common.sh` evals brew shellenv on source).
